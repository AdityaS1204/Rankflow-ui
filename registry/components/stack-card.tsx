...
const defaultItems: StackCardItem[] = [
  {
    ...
    // Premium Apple-like hardware/macro photography with ample negative space
  ...
  },
  {
    ...
    // Clean minimalist tech placement
...
export function StackCard({ items = defaultItems }: { items?: StackCardItem[] }) {
  ...
  useEffect(() => {
    ...
    }, 4500); // 4.5 seconds interval for a premium, unhurried feel
  ...
  return (
    <div className="relative w-full max-w-[320px] aspect-3/4 mx-auto perspective-1000">
      {cards.map((card, index) => {
        ...
        const yOffset = index * 24; // Push down 24px per index so the top stacks
        const zIndex = cards.length - index;
        const opacity = index > 2 ? 0 : 1; // Only show top 3 cards for performance and visual clarity
        ...
        return (
          <motion.div
            ...
            className={`absolute inset-0 w-full h-full rounded-[36px] overflow-hidden flex shadow-2xl will-change-transform ${
              // The frosted glass effect is applied to all cards. 
              // The card in front blurs the images of the cards behind it!
            ...
            >
              ...
              <div
                className={`flex-1 w-full rounded-[24px] overflow-hidden relative shadow-inner ${
                  // Add a subtle inner light boundary for premium feel
